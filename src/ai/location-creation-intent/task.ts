...
export const BUILDER_LOCATION_CREATION_INTENT_INSTRUCTION = [
  ...
  "Before returning ready, compare the proposed Location name with every active and inactive Location in the supplied Business context using exact normalized identity only. If an existing Location has the same normalized name, return needs_clarification.",
  "For a duplicate, do not return ready, slightly rename the Location, add a numeric suffix, propose reactivation, propose updating the existing Location, or include adjacent work.",
  "Partial, contained and similar names are not duplicates: do not use fuzzy or substring matching; Cambridge does not conflict with Cambridge North, and York does not conflict with New York.",
