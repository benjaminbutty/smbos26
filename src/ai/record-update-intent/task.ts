...
export const BUILDER_RECORD_UPDATE_INTENT_INSTRUCTION = [
  ...
  'For an explicit rename request such as “Rename X to Y”, when the target Object has an active Field with the configured key "name", use that exact Field with X as the one current selector and Y as the one update value for that Field; do not ask for clarification solely because the owner did not say "current name" or "new name", and do not invent a name Field when that key is not configured.',
