...
| `src/ai/builder/` | Authenticated server-only Builder composition of authoritative context, qualified planning, private qualified generic drafting, and the disabled-until-gated preorder amendment task through the existing proposal handoff; sequential metadata-only accounting; no direct compiler, lifecycle, route/UI or operational mutation |
