-- Milestone 6 Phase 2A.1
-- Ordinary proposals must be composed against the exact immutable active
-- version and head revision rendered to the owner. PostgreSQL performs the
-- authoritative comparison while holding the existing head lock.
...
comment on function public.propose_configuration_change(
  ...
  'Owner/Admin ordinary proposal boundary. It locks the Business head, rejects an obsolete expected active version or revision, and authoritatively materialises one immutable Milestone 5 proposal.';
